...
def multi_map(arr, n= 1, &prc)
    ...
    arr.each do |ele|
        result = ele        #don't need this line if the next has prc.call(ele) in it
        n.times {result = prc.call(result)}
        new_arr << result
    end
    return new_arr
end
...
def selected_map!(arr, prc, prc2)
    arr.map! do |ele|
        prc.call(ele) ? prc2.call(ele) : ele
    end
    #     if prc.call(ele)
    #         prc2.call(ele)
    #     else
...
def chain_map(val, prc_arr)
    return prc_arr[0].call(val) if prc_arr.length == 1
    chain_map(prc_arr.first.call(val), prc_arr[1..prc_arr.length - 1])
end
...
******
def proc_suffix(sentence, hash)         #odd that current_word needs to say duplicated next to it, otherwise it would modify the word when changed ***
    words = sentence.split
    new_words = []
    words.each do |word|
        current_word = word.dup
        #debugger
        if hash.keys.any? {|key| key.call(word)}
            hash.keys.each do |prc|
                current_word << hash[prc] if prc.call(word)
            end
        end
...
def proctition_platinum(arr, *prcs)
    ...
    prcs.each.with_index do |prc, index|
        hash[index + 1] = arr.select {|ele| prcs[index].call(ele) && !hash.values.flatten.include?(ele)}
    end
    return hash
end
    ...
    words.each do |word|
        ...
        if hash.keys.any? {|key| key.call(word)}
            hash.each_pair do |k,v|
                if k.call(word)
                    current_word = v.call(current_word)
                end
            end
        end
...
def picky_procipher(sentence, hash)
    ...
    words.each do |word|
        current_word = word.dup
        if hash.keys.any? {|key| key.call(word)}
            applicable_keys = hash.keys.select {|key| key.call(word)}
            current_word = hash[applicable_keys[0]].call(word)
        end
        new_sentence << current_word
    end
    return new_sentence.join(" ")
